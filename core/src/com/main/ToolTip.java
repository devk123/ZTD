...
public class ToolTip {
    ...
    ToolTip(String type, Button p){
        ...
        x = (p.x + p.w / 2) - w / 2;
        y = p.y - h - 15;
        close = new Button("close", x + w - 25 - 1, y + h - 25 - 1);
        close.locked = false;
    }
    ...
    void draw(SpriteBatch batch){
        ...
        font.draw(batch, "Unlock: " + (Tables.values.get("unlock_"+type) == null ? "0" : Tables.values.get("unlock_"+type)), x + 35 + 1, y + 45 - 1);
        font.setColor(Color.FOREST);
        font.draw(batch, "Unlock: " + (Tables.values.get("unlock_"+type) == null ? "0" : Tables.values.get("unlock_"+type)), x + 35, y + 45);
